...
document.getElementById('submitLinkBtn').addEventListener('click', async () => {
    ...
    try {
        await db.collection('youtubeLinks').doc(videoId).set({
            ...
            title: `Video Title (ID: ${videoId})`, // In a real app, fetch this from YouTube Data API
...
let adsWatchedCountSession = 0;
document.getElementById('watchAdsBtn').addEventListener('click', async () => {
    const adsStatus = document.getElementById('adsStatus');
    adsStatus.textContent = 'Preparing ads...';
    
    // Simulate watching 4 ads sequentially
    for (let i = 0; i < 4; i++) {
        adsStatus.textContent = `Watching ad ${i + 1} of 4. Please wait...`;
        
        // Call Monetag Rewarded Interstitial
        try {
            // show_10276123() is the function from the Monetag SDK
            await show_10276123(); 
            // User watched the ad till the end or closed it in interstitial format
            updateBalance(REWARD_PER_AD);
            adsWatchedCountSession++;
            adsStatus.textContent = `Ad ${i + 1} watched! Earned ${REWARD_PER_AD.toFixed(2)}⚡ Peso. Total: ${pesoBalance.toFixed(2)}⚡ Peso.`;
            await new Promise(resolve => setTimeout(resolve, 1500)); // Brief pause between ads
        } catch (e) {
            ...
            console.error('Monetag ad error or user closed ad:', e);
            alert(`Ad ${i + 1} was not completed. You didn't receive a reward for this ad.`);
            break; // Stop if an ad fails or is prematurely closed
        }
    }
    adsStatus.textContent += ` All ${adsWatchedCountSession} ads watched successfully! Total earned from this session: ${(REWARD_PER_AD * adsWatchedCountSession).toFixed(2)}⚡ Peso.`;
    adsWatchedCountSession = 0; // Reset for next session
});
...
function setupWithdrawalHistoryListener() {
    ...
    db.collection('withdrawals')
        ...
        .onSnapshot(snapshot => {
            ...
            let html = '';
            const tenDaysAgo = Date.now() - (10 * 24 * 60 * 60 * 1000); // For 10-day display if needed, but showing all for "forever"
            snapshot.docs.forEach(doc => {
                const data = doc.data();
                const requestedTime = data.requestedAt ? new Date(data.requestedAt.toDate()) : new Date();
                // If you wanted to filter for 10 days:
                // if (requestedTime.getTime() < tenDaysAgo) return; 
...
function setupAdminWithdrawalsListener() {
    ...
    db.collection('withdrawals')
        ...
        .onSnapshot(snapshot => {
            ...
            let html = '';
            // const tenDaysAgo = Date.now() - (10 * 24 * 60 * 60 * 1000); // For 10-day display if needed
            snapshot.docs.forEach(doc => {
                const data = doc.data();
                const requestedTime = data.requestedAt ? new Date(data.requestedAt.toDate()) : new Date();
                // if (requestedTime.getTime() < tenDaysAgo) return; 
